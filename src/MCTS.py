...
class MonteCarloTreeSearchCheckpoint():
    def __init__(self, state,env,memory, parent=None, parent_action=None, filter_prop=None, filter_moves=None, engine=None):
        self.state = copy.deepcopy(state)
        self.env = copy.deepcopy(env)
        self.memory = memory
        self.parent = parent
    ...
    def is_game_over(self, state, cnt=0):
        '''
        It is the game over condition
        and depends on your game. Returns
        true or false
    ...
    def expand(self):
        ...
        #print(next_env.render())
        #print(' ')
        child_node = MonteCarloTreeSearchCheckpoint(
                    next_state, parent=self, parent_action=action, env=next_env, memory=self.memory, engine=self.engine)
    ...
    def rollout(self):
        ...
        material_adv_list = []
        cnt = 0
        #print(self.is_game_over(current_rollout_state, cnt))
        while not self.is_game_over(current_rollout_state, cnt):
            #possible_moves = current_rollout_state.get_legal_actions()
            possible_moves = self.get_legal_actions(current_rollout_state,current_rollout_env)
            
            action = self.rollout_policy(possible_moves,current_rollout_state,current_rollout_env)
            #current_rollout_state = current_rollout_state.move(action)
            self.steps.append(action)
    ...
    def best_child(self, c_param=0.1, par=None):
        ...
        #print([(c.q()) for c in self.children])
        #print([(c.n()) for c in self.children])
    ...
    def rollout_policy(self, possible_moves, cstate,cenv):
        '''
        Chooses the moves to visit
        '''
        n_move = possible_moves[np.random.randint(len(possible_moves))]
        
        if self.engine is not None:
            '''
            moves_anal = []
            for m in possible_moves:
                n_state = copy.deepcopy(cstate)
                n_env = copy.deepcopy(cenv)
                action = n_state.push_san(m)
                n_state,reward,done,_ = n_env.step(action)
                
                evaluation = self.engine.analyse(n_state, chess.engine.Limit(time=0.1))
                mate = evaluation["score"].white().mate()
                evaluation = evaluation["score"].white().score()
                if mate != None:
                    evaluation = -(20-mate) * 1000
                evaluation = evaluation
                moves_anal.append(evaluation)
                
            n_move = possible_moves[np.argmax(moves_anal)]'''
            result = self.engine.play(cstate, chess.engine.Limit(time=0.01))
            n_move = cstate.san(result.move)
            #print("Analytics moves: ", moves_anal, " move: ", n_move)
...
def calculate_MCTS_checkpoint(state, env, memory, filter_prop = None ,filter_moves = None, par=None, engine=None):
    #df1 = pd.read_pickle("../data/pgn/chess_deep_memory.pkl")
    root = MonteCarloTreeSearchCheckpoint(state = state, env = env, memory = memory, filter_prop = filter_prop,filter_moves = filter_moves, engine=engine)
    selected_node,weights,material_adv  = root.best_action(par)
